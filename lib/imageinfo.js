		...
		return {
					...
					this.loadFile(url, function(){
						...
						fs.onload = (function(self) {
							return function(e) {
									...
									_this.tags['dataURI'] = data_uri;
									self.setImage(data_uri, callback);
						        };
						})(_this);
			...
			setImage : function(src, callback){
				...
				image.addEventListener('load', function(){
					...
					_this.canvas.width = src_w;
					_this.canvas.height = src_h;
					var ctx = _this.canvas.getContext('2d');
					_this.context = ctx;
					ctx.drawImage(img, 0, 0, src_w, src_h);
					_this.imageData = ctx.getImageData(0, 0, src_w, src_h);
			...
			setColorMap : function(callback, _this){
				if (this.conf.useWorker){
					var worker = new Worker('/lib/worker.colormap.js');
